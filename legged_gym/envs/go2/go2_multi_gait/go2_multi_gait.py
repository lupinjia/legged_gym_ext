...
class GO2MultiGait(LeggedRobot):
    ...
    def post_physics_step(self):
        ...
        self._update_dof_history_obs()
        
        if self.viewer and self.enable_viewer_sync and self.debug_viz:
            self._draw_debug_vis()

    def _update_base_history_obs(self):
        """ Update base history observation
        """
        self.l4ast_commands[:] = self.l3ast_commands[:]
        self.l3ast_commands[:] = self.llast_commands[:]
        self.llast_commands[:] = self.last_commands[:]
        self.last_commands[:] = self.commands[:]
        self.l4ast_projected_gravity[:] = self.l3ast_projected_gravity[:]
        self.l3ast_projected_gravity[:] = self.llast_projected_gravity[:]
        self.llast_projected_gravity[:] = self.last_projected_gravity[:]
        self.last_projected_gravity[:] = self.projected_gravity[:]
        self.l4ast_base_ang_vel[:] = self.l3ast_base_ang_vel[:]
        self.l3ast_base_ang_vel[:] = self.llast_base_ang_vel[:]
        self.llast_base_ang_vel[:] = self.last_base_ang_vel[:]
        self.last_base_ang_vel[:] = self.base_ang_vel[:]
        self.l4ast_clock_input[:] = self.l3ast_clock_input[:]
        self.l3ast_clock_input[:] = self.llast_clock_input[:]
        self.llast_clock_input[:] = self.last_clock_input[:]
        self.last_clock_input[:] = self.clock_input[:]
        self.l4ast_phase_ratio[:] = self.l3ast_phase_ratio[:]
        self.l3ast_phase_ratio[:] = self.llast_phase_ratio[:]
        self.llast_phase_ratio[:] = self.last_phase_ratio[:]
        self.last_phase_ratio[:] = self.phase_ratio[:]
    
    def _update_dof_history_obs(self):
        """ Update dof history observation
        """
        self.l4ast_dof_pos[:] = self.l3ast_dof_pos[:]
        self.l3ast_dof_pos[:] = self.llast_dof_pos[:]
        self.llast_dof_pos[:] = self.last_dof_pos[:]
        self.last_dof_pos[:] = self.dof_pos[:]

        self.l4ast_dof_vel[:] = self.l3ast_dof_vel[:]
        self.l3ast_dof_vel[:] = self.llast_dof_vel[:]
        self.llast_dof_vel[:] = self.last_dof_vel[:]
        self.last_dof_vel[:] = self.dof_vel[:]

        self.l4ast_actions[:] = self.l3ast_actions[:]
        self.l3ast_actions[:] = self.llast_actions[:]
    ...
    def reset_buffers(self, env_ids):
        """ Reset some buffers
        """
        self.l4ast_commands[env_ids] = 0.
        self.l3ast_commands[env_ids] = 0.
        self.llast_commands[env_ids] = 0.
        self.last_commands[env_ids] = 0.
        self.l4ast_projected_gravity[env_ids] = 0.
        self.l3ast_projected_gravity[env_ids] = 0.
        self.llast_projected_gravity[env_ids] = 0.
        self.last_projected_gravity[env_ids] = 0.
        self.l4ast_base_ang_vel[env_ids] = 0.
        self.l3ast_base_ang_vel[env_ids] = 0.
        self.llast_base_ang_vel[env_ids] = 0.
        self.last_base_ang_vel[env_ids] = 0.
        self.l4ast_dof_pos[env_ids] = 0.
        self.l3ast_dof_pos[env_ids] = 0.
        self.llast_dof_pos[env_ids] = 0.
        self.last_dof_pos[env_ids] = 0.
        self.l4ast_dof_vel[env_ids] = 0.
        self.l3ast_dof_vel[env_ids] = 0.
        self.llast_dof_vel[env_ids] = 0.
        self.last_dof_vel[env_ids] = 0.
        self.l4ast_actions[env_ids] = 0.
        self.l3ast_actions[env_ids] = 0.
        self.llast_actions[env_ids] = 0.
        self.last_actions[env_ids] = 0.
        self.l4ast_clock_input[env_ids] = 0.
        self.l3ast_clock_input[env_ids] = 0.
        self.llast_clock_input[env_ids] = 0.
        self.last_clock_input[env_ids] = 0.
        self.l4ast_phase_ratio[env_ids] = 0.
        self.l3ast_phase_ratio[env_ids] = 0.
        self.llast_phase_ratio[env_ids] = 0.
        self.last_phase_ratio[env_ids] = 0.
    ...
    def _reset_dofs(self, env_ids):
        ...
        if self.cfg.init_state.init_state_train:
            ...
            dof_pos[:, [1, 4, 7, 10]] = self.default_dof_pos[:,[1, 4, 7, 10]] + torch_rand_float(-0.4, 0.4, (len(env_ids), 4), device=self.device) # thigh
            dof_pos[:, [2, 5, 8, 11]] = self.default_dof_pos[:,[2, 5, 8, 11]] + torch_rand_float(-0.4, 0.4, (len(env_ids), 4), device=self.device) # calf
    ...
    def _init_buffers(self):
        ...
        self.omega_walking = torch.zeros(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
        # observation history
        self.last_commands = torch.zeros_like(self.commands)
        self.llast_commands = torch.zeros_like(self.commands)
        self.l3ast_commands = torch.zeros_like(self.commands)
        self.l4ast_commands = torch.zeros_like(self.commands)
        self.last_projected_gravity = torch.zeros_like(self.projected_gravity)
        self.llast_projected_gravity = torch.zeros_like(self.projected_gravity)
        self.l3ast_projected_gravity = torch.zeros_like(self.projected_gravity)
        self.l4ast_projected_gravity = torch.zeros_like(self.projected_gravity)
        self.last_base_ang_vel = torch.zeros_like(self.base_ang_vel)
        self.llast_base_ang_vel = torch.zeros_like(self.base_ang_vel)
        self.l3ast_base_ang_vel = torch.zeros_like(self.base_ang_vel)
        self.l4ast_base_ang_vel = torch.zeros_like(self.base_ang_vel)

        self.last_dof_pos = torch.zeros_like(self.dof_pos)
        self.llast_dof_pos = torch.zeros_like(self.dof_pos)
        self.l3ast_dof_pos = torch.zeros_like(self.dof_pos)
        self.l4ast_dof_pos = torch.zeros_like(self.dof_pos)

        self.llast_dof_vel = torch.zeros_like(self.dof_vel)
        self.l3ast_dof_vel = torch.zeros_like(self.dof_vel)
        self.l4ast_dof_vel = torch.zeros_like(self.dof_vel)

        self.l3ast_actions = torch.zeros_like(self.actions)
        self.l4ast_actions = torch.zeros_like(self.actions)
        ...
        self.gait_period[:] = self.cfg.rewards.periodic_reward_framework.gait_period[1]
        ...
        self.phase_ratio[:, 0] = self.cfg.rewards.periodic_reward_framework.swing_phase_ratio[0]
        ...
        self.last_phase_ratio = torch.zeros_like(self.phase_ratio)
        self.llast_phase_ratio = torch.zeros_like(self.phase_ratio)
        self.l3ast_phase_ratio = torch.zeros_like(self.phase_ratio)
        self.l4ast_phase_ratio = torch.zeros_like(self.phase_ratio)
        #----- Quad2Upright -----#
        self.upright_indicator = 1/(1+torch.exp(-200*(self.rpy[:, 1] - 1.0)))  # 1.4rad(80deg). For pitch > -80deg, indicator=1, else indicator=0.
    ...
    def _parse_cfg(self, cfg):
        ...
        self.num_history_timesteps = self.cfg.env.num_history_timesteps
        self.num_history_obs_single_step = self.cfg.env.num_history_obs_single_step
    ...
    def _reward_tracking_lin_vel(self):
        ...
        lin_vel_x_error = torch.square(self.commands[:, 0] - self.base_lin_vel[:, 0])  # when upright, x_vel is -z_vel
        lin_vel_y_error = torch.square(self.commands[:, 1] - self.base_lin_vel[:, 1])
        lin_vel_error = torch.sqrt(lin_vel_x_error + lin_vel_y_error)
        zero_vel_x_error = torch.square(self.base_lin_vel[:, 0])
        zero_vel_y_error = torch.square(self.base_lin_vel[:, 1])
        zero_vel_error = torch.sqrt(zero_vel_x_error + zero_vel_y_error)
        # return torch.exp(-lin_vel_error/self.tracking_sigma) * self.omega_walking.squeeze(-1) + (1-self.omega_walking.squeeze(-1)) * torch.exp(-zero_vel_error/self.tracking_sigma)
    ...
    def _reward_tracking_ang_vel(self):
        ...
        zero_ang_vel_error = torch.square(self.base_ang_vel[:, 2])
        # return torch.exp(-ang_vel_error/self.tracking_sigma) * self.omega_walking.squeeze(-1) + (1-self.omega_walking.squeeze(-1)) * torch.exp(-zero_ang_vel_error/self.tracking_sigma)
    ...
    def _reward_standing_cost(self):
        ...
        reward = (1-self.omega_walking.squeeze(-1))*(action_diff*20 + legs_sym_error)
        return reward
