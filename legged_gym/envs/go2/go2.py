...
class GO2(LeggedRobot):
    ...
    def post_physics_step(self):
        ...
        # state indicator, for upright reward
        self.upright_indicator = 1/(1+torch.exp(-200*(self.rpy[:, 1] - 1.0)))
    ...
    def check_upright_dof_pos_termination(self):
        """ Check if the robot's specific dof positions are out of the specified range
        """
        # time_within_threshold = self.episode_length_buf < (3.0 / self.dt)
        hip_joint_indice = [0, 3, 6, 9]
        dof_pos_termination = torch.any(torch.abs(self.dof_pos[:, hip_joint_indice] - self.default_dof_pos[:, hip_joint_indice]) > 0.5, dim=1)
        # self.reset_buf |= torch.logical_and(time_within_threshold, dof_pos_termination)
        self.reset_buf |= dof_pos_termination
    
    def check_upright_termination(self):
        '''check if the robot is upright(base_angle, base_height) after 3s'''
        time_over_threshold = self.episode_length_buf > (3.0 / self.dt)
        pitch_angle_out_of_range = torch.logical_or(self.rpy[:, 1] < 1.0, self.rpy[:, 1] > 1.6)  # pitch angle must be within [-1.74, -1.4]rad([-100, -80]deg)
        base_height_over_limit = self.base_pos[:, 2] > 0.60   # base height must be below 0.60m
        base_height_below_limit = self.base_pos[:, 2] < 0.40  # base height must be above 0.45m
        self.upright_termination = torch.logical_and(time_over_threshold, torch.logical_or(pitch_angle_out_of_range, base_height_below_limit))
        self.upright_termination |= base_height_over_limit
        self.reset_buf |= self.upright_termination
    ...
    def check_termination(self):
        ...
        # if self.cfg.env.train_quad2upright:
            # self.check_upright_termination()
            # self.check_upright_dof_pos_termination()
    ...
    def _reset_dofs(self, env_ids):
        ...
        if self.cfg.init_state.init_state_train:
            ...
            dof_pos[:, [1, 4, 7, 10]] = self.default_dof_pos[:,[1, 4, 7, 10]] + torch_rand_float(-1.0, 1.0, (len(env_ids), 4), device=self.device) # thigh
            dof_pos[:, [2, 5, 8, 11]] = self.default_dof_pos[:,[2, 5, 8, 11]] + torch_rand_float(-0.6, 0.6, (len(env_ids), 4), device=self.device) # calf
            self.dof_pos[env_ids] = dof_pos
            # self.dof_pos[env_ids] = self.default_dof_pos * torch_rand_float(0.5, 1.5, (len(env_ids), self.num_dof), device=self.device)
            self.dof_vel[env_ids] = 0.
            # self.dof_vel[env_ids] = torch_rand_float(-2., 2., (len(env_ids), self.num_dof), device=self.device)
    ...
    def _reset_root_states(self, env_ids):
        ...
        # base orientation
        if self.cfg.init_state.randomize_base_orientation:
            self.root_states[env_ids, 3:7] = quat_from_euler_xyz(torch.zeros(len(env_ids), device=self.device), 
                                                                 torch_rand_float(0, torch.pi/4, (len(env_ids),1), device=self.device).view(-1), 
    ...
    def compute_observations(self):
        ...
        """
        self.obs_buf = torch.cat(( self.commands[:, :3] * self.commands_scale,                           # 3
                                   self.rpy * self.obs_scales.rpy,                                       # 3
    ...
    def _get_noise_scale_vec(self):
        ...
        noise_vec[3:6] = noise_scales.rpy * noise_level * self.obs_scales.rpy
    ...
    def _init_buffers(self):
        ...
        if self.biped_enable:
            self.gait_period[:] = self.cfg.rewards.periodic_reward_framework.biped.gait_period
            self.theta = torch.zeros(self.num_envs, 4, dtype=torch.float, device=self.device, requires_grad=False)
            self.theta[:, 0] = self.cfg.rewards.periodic_reward_framework.biped.theta_left[0]
            self.theta[:, 1] = self.cfg.rewards.periodic_reward_framework.biped.theta_right[0]
            self.phase_ratio[:, 0] = self.cfg.rewards.periodic_reward_framework.biped.swing_phase_ratio[0] # default is the first value
            self.phase_ratio[:, 1] = self.cfg.rewards.periodic_reward_framework.biped.stance_phase_ratio[0]
            self.b_swing[:, :] = self.cfg.rewards.periodic_reward_framework.biped.b_swing[0] *2*torch.pi
        # elif self.quad_enable:
        #     self.theta = torch.zeros(self.num_envs, 4, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        # self.omega_walking = 1/(1+torch.exp(-200*(self.phase_ratio[:, 0] - 0.15)))  # use swing phase ratio to indicate gait
    ...
    def _parse_cfg(self, cfg):
        ...
        self.biped_enable = self.cfg.rewards.periodic_reward_framework.biped.enable
        assert ~(self.quad_enable and self.biped_enable), "Cannot enable both quadruped and biped gaits at the same time"
        if self.biped_enable:
            self.a_swing = self.cfg.rewards.periodic_reward_framework.biped.a_swing * 2*torch.pi
            self.b_stance = self.cfg.rewards.periodic_reward_framework.biped.b_stance * 2*torch.pi
            self.selected_gait = self.cfg.rewards.periodic_reward_framework.biped.selected_gait
        # elif self.quad_enable:
    ...
    def _reward_tracking_lin_vel(self):
        ...
        lin_vel_x_error = torch.square(self.commands[:, 0] - self.base_lin_vel[:, 2])  # when upright, x_vel is -z_vel
        lin_vel_y_error = torch.square(self.commands[:, 1] - self.base_lin_vel[:, 1])
        lin_vel_error = torch.sqrt(lin_vel_x_error + lin_vel_y_error)
        # return torch.exp(-lin_vel_error/self.tracking_sigma * self.omega_walking)
        return torch.exp(-lin_vel_error/self.tracking_sigma) * self.upright_indicator  # activate when indicator=1(pitch < -80deg, upright)
        # return torch.exp(-lin_vel_error/self.tracking_sigma) # for mode-aware function ablation study
    
    def _reward_tracking_ang_vel(self):
        # Tracking of angular velocity commands (yaw)
        ang_vel_error = torch.square(self.commands[:, 2] - self.base_ang_vel[:, 2]) # when upright, rotatiion_vel is still yaw_vel
        # return torch.exp(-ang_vel_error/self.tracking_sigma * self.omega_walking)
        return torch.exp(-ang_vel_error/self.tracking_sigma) * self.upright_indicator  # activate when indicator=0(pitch < -80deg, upright)
    ...
    def calc_periodic_reward_obs(self):
        ...
        elif self.biped_enable:
            # 和quadruped的维度保持一致
            self.clock_input = torch.zeros((self.num_envs, 4), device=self.device)
            clock_input_left = torch.sin(2*torch.pi*(self.phi+self.theta[:, 0].view(-1, 1)) / num_cycle_timesteps).flatten()
            clock_input_right = torch.sin(2*torch.pi*(self.phi+self.theta[:, 1].view(-1, 1)) / num_cycle_timesteps).flatten()
            self.clock_input[:, 0] = clock_input_left  # 前两个维度放双足两条腿的时钟信号
            self.clock_input[:, 1] = clock_input_right
    ...
    def resample_phase_and_theta(self, env_ids):
        ...
        if self.biped_enable:
            ...
            self.gait_period[env_ids, 0] = self.cfg.rewards.periodic_reward_framework.biped.gait_period
            # update theta
            self.theta[env_ids, 0] = self.cfg.rewards.periodic_reward_framework.biped.theta_left[0]
            self.theta[env_ids, 1] = self.cfg.rewards.periodic_reward_framework.biped.theta_right[0]
            # update b_swing, phase ratio
            self.b_swing[env_ids, 0] = self.cfg.rewards.periodic_reward_framework.biped.b_swing[0] *2*torch.pi
            self.a_stance = self.b_swing
            self.phase_ratio[env_ids, 0] = self.cfg.rewards.periodic_reward_framework.biped.swing_phase_ratio[0]
            self.phase_ratio[env_ids, 1] = self.cfg.rewards.periodic_reward_framework.biped.stance_phase_ratio[0]
    ...
    # def change_mode(self, target_mode="quad"):
    #     if target_mode == "quad":
    #         if self.quad_enable == True and self.biped_enable == False: # 原本就是四足
    #             pass
    #         else: # 其它情况切换到四足
    #             self.quad_enable = True
    #             self.biped_enable = False
    #             # 更新步态周期, 重置步态时间
    #             self.gait_period = self.cfg.rewards.periodic_reward_framework.quadruped.gait_period
    #             self.gait_time[:, :] = 0.0
    #     elif target_mode == "biped":
    #         if self.quad_enable == False and self.biped_enable == True: # 原本就是两足
    #             pass
    #         else: # 其它情况切换到两足
    #             self.quad_enable = False
    #             self.biped_enable = True
    #             # 更新步态周期, 重置步态时间
    #             self.gait_period = self.cfg.rewards.periodic_reward_framework.biped.gait_period
    #             self.gait_time[:, :] = 0.0
    ...
    def _reward_periodic_reward(self):
        if self.quad_enable:
            ...
            # self.calc_contact_indicator()
            # reward for the whole body
            reward = reward_fl.flatten() + reward_fr.flatten() + reward_rl.flatten() + reward_rr.flatten()
        elif self.biped_enable:
            # reward for each foot
            reward_left, self.exp_C_spd_left, self.exp_C_frc_left = self.uniped_periodic_reward("left")
            reward_right, self.exp_C_spd_right, self.exp_C_frc_right = self.uniped_periodic_reward("right")
            reward = reward_left.flatten() + reward_right.flatten()
        # return torch.exp(reward / 2.0) # for mode-aware function ablation study
        return torch.exp(reward / 2.0) * self.upright_indicator  # activate when indicator=1(pitch < -80deg, upright)
    
    def _reward_base_angle_upright(self):
        '''reward for base angle upright'''
        roll_error = torch.square(self.rpy[:, 0])  # roll->0
        pitch_error = torch.square(self.rpy[:, 1] - self.cfg.rewards.base_angle_upright_target)  # pitch->-pi/2 when upright
        return torch.exp(-(roll_error + pitch_error))
    
    def _reward_base_height_upright(self):
        '''reward for base height upright'''
        height_error = torch.square(self.base_pos[:, 2] - self.cfg.rewards.base_height_upright_target)  # height->target
        return torch.exp(-height_error)
    
    def _reward_upright_contacts(self):
        '''reward for upright contacts'''
        rear_contacts = self.contact_forces[:, [self.foot_index_rl, self.foot_index_rr], 2] > 0.1  # F_z > 0.1 is contact
        # front legs are not in contact and rear legs are no more than 2 contacts
        upright_contacts = torch.sum(1.0*rear_contacts, dim=1) == 0
        return 1.0*upright_contacts
    
    # def _reward_standing_cost(self):
    #     '''reward for standing cost
    #     penalize foot z pos and action diff when standing
    #     '''
    #     action_diff = torch.sum(torch.square(self.last_actions - self.actions), dim=-1)
    #     front_legs_xz_error = torch.sum(torch.square(self.foot_pos_fl[:, [0, 2]] - self.foot_pos_fr[:, [0, 2]]), dim=-1)
    #     rear_legs_xz_error = torch.sum(torch.square(self.foot_pos_rl[:, [0, 2]] - self.foot_pos_rr[:, [0, 2]]), dim=-1)
    #     left_legs_y_error = torch.sum(torch.square(self.foot_pos_fl[:, 1] - self.foot_pos_rl[:, 1]), dim=-1)
    #     right_legs_y_error = torch.sum(torch.square(self.foot_pos_fr[:, 1] - self.foot_pos_rr[:, 1]), dim=-1)
    #     legs_sym_error = front_legs_xz_error + rear_legs_xz_error + left_legs_y_error + right_legs_y_error
    #     reward = (1-self.omega_walking)*(action_diff*20 + legs_sym_error)
    #     return reward
    ...
    def _reward_lower_legs_cost(self):
        '''Penalize the upper legs motion when walking upright'''
        # hip:0.0, thigh: 2.0, calf: -0.9
        rear_legs_dof_indices = [6, 7, 8, 9, 10, 11]
        rear_legs_pos_cost = torch.sum(torch.square(self.dof_pos[:, rear_legs_dof_indices] - self.default_dof_pos[:, rear_legs_dof_indices]), dim=-1)
        return rear_legs_pos_cost
    
    def _reward_ang_vel_x(self):
        '''Penalize angular velocity of base xz'''
