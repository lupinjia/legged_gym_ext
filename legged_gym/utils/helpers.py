...
def update_estimator_cfg_from_args(cfg_estimator, args):
    if cfg_estimator is not None:
        if args.seed is not None:
            cfg_estimator.seed = args.seed
        # alg runner parameters
        if args.max_iterations is not None:
            cfg_estimator.runner.max_iterations = args.max_iterations
        if args.resume:
            cfg_estimator.runner.resume = args.resume
        if args.experiment_name is not None:
            cfg_estimator.runner.experiment_name = args.experiment_name
        if args.run_name is not None:
            cfg_estimator.runner.run_name = args.run_name
        if args.load_run is not None:
            cfg_estimator.runner.load_run = args.load_run
        if args.checkpoint is not None:
            cfg_estimator.runner.checkpoint = args.checkpoint
    
    return cfg_estimator
